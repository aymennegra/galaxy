<?PHP
include "C:/wamp64/www/galaxy/config.php";
class livraisonC {
function afficherlivraison ($livraison){
	...
	function ajouterlivraison($livraison){
		$sql="insert into livraison (idclient,nom,adresse,numtel,numcommande,liv) values (:idclient, :nom,:adresse,:numtel,:numcommande,:liv)";
	...
	function modifierlivraison($livraison,$idclient){
		$sql="UPDATE livraison SET idclient=:idclientn, nom=:nom,adresse=:adresse,numtel=:numtel,numcommande=:numcommande,liv=:liv WHERE idclient=:idclient";
...
try{		
		...
		$numcommande=$livraison->getnumcommande();
		$liv=$livraison->getliv();
		$datas = array(':idclientn'=>$idclientn, ':idclient'=>$idclient, ':nom'=>$nom,':adresse'=>$adresse,':numtel'=>$numtel,':numcommande'=>$numcommande,':liv'=>$liv);
